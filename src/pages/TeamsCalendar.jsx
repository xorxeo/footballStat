...
export const TeamsCalendar = () => {
    ...
    if (data) {
      ...
      if (searchDateStart && searchDateEnd) {
        matches = data.matches
          .filter((match) => isInRange(searchDateStart, searchDateEnd, match.utcDate))
          .map((match) => {
            return [
              match.homeTeam.name,
              match.awayTeam.name,
  ...
  return (
    <div className="teams-calendar">
      ...
      {data && <Breadcrumbs crumbs={crumbs} data={bodyData} />}
      {data && <SimpleTable data={bodyData} heads={TABLE_HEADS} />}
    </div>
  );
};
